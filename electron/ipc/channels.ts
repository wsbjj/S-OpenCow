...
export function registerIPCHandlers(deps: IPCDeps): void {
  ...
  if (orchestrator) {
    ...
    registerHandler('command:compact-session', async (sessionId: string) => {
      return await orchestrator.compactSession(sessionId, 'manual')
    })
    registerHandler('command:set-session-reasoning-effort', (sessionId, effort) =>
      orchestrator.setSessionReasoningEffort(sessionId, effort)
    )
    ...
    registerHandler('command:get-session-message-page', (sessionId, params) =>
      orchestrator.getStore().getMessagePage(sessionId, params)
    )
    registerHandler('command:search-session-messages', (sessionId, query, limit) =>
      orchestrator.getStore().searchSessionMessages(sessionId, query, limit)
    )
