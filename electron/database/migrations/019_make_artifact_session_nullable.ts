...
/**
 * Migration 019: make artifacts.session_id nullable.
 *
 ...
 *
 * NOTE: This migration was originally drafted as 017_make_artifact_session_nullable.ts
 * but was never registered in provider.ts (it was shadowed by 017_add_origin_extra.ts).
 * Registered here as 019 — the next available slot after 018_create_issue_context_refs.
 *
...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  // 5. Recreate indexes (same as migration 006, updated for nullable session_id)
  await db.schema
...
export async function down(db: Kysely<unknown>): Promise<void> {
  // Restore to NOT NULL — rows with null session_id get empty string sentinel
