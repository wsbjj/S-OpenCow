...
type Props = CapabilityFormProps<RuleFormFields>

export function RuleForm({ mode, saving, onSave, onCancel, onDirty, variant }: Props): React.JSX.Element {
  const s = formStyles(variant)
  const initial =
    mode.type === 'edit' ? mode.initialData : { name: '', description: '', body: '' }
  ...
  return (
    <div className="flex flex-col h-full">
      <div className={s.header}>
        ...
        <div>
          <label htmlFor="rule-name" className={s.label}>
            Name
          </label>
          <input
            id="rule-name"
        ...
        <div>
          <label htmlFor="rule-desc" className={s.label}>
            Description
          </label>
          <input
            id="rule-desc"
